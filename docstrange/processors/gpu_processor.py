...
class GPUConversionResult(ConversionResult):
    ...
    def extract_data(self, specified_fields: Optional[list] = None, json_schema: Optional[dict] = None, 
        ...
        print(f"specified_fields provided: {specified_fields is not None}")
    ...
    def _extract_json_with_model(self, json_schema: Optional[dict] = None) -> Dict[str, Any]:
        ...
        Args:
            json_schema: Optional JSON schema to guide extraction (like cloud version)
        ...
        try:
            ...
            if json_schema:
                # Schema-guided extraction (like cloud version with "specified-json")
                schema_str = json.dumps(json_schema, indent=2)
                prompt = f"""Extract information from the above document and return it as a valid JSON object that matches the following schema.

JSON Schema:
{schema_str}

Instructions:
- The output should be a single JSON object matching the schema structure.
...
- For array fields, return a list of objects.
...
Example:
{
  ...
  "watermark": "CONFIDENTIAL"
            ...
            if json_schema:
                # Schema-guided extraction format (matches cloud "specified-json")
                result = {
